...
memolist: .space 400 # Espacio para almacenar resultados, asumiendo que n no será mayor a 200 (Esto puede modificarse)
...
main:
 # Solicitud al usuario de que ingrese un numero
    ...
    li $a2, 400		# Inicializa el tamaño del registro que tiene dentro memlist
    li $t1, 0		# Inicializa el valor inicial del registro en cero (como lo dice el enunciado)

init_memolist: # Etiqueta init_memlist
    sw $t1, 0($a1)	# Escribe en $t1 la direccion de memoria de la posicion 0($a1), es decir, primero suma el valor de 0 con la direccion de $a1 y luego lo almacena en el registro correspondiente para ejecutar escritura
...
memo_hit: #Etiqueta memo_hit
    lw $v0, 0($t3)        # Cargamos el valor almacenado en memolist[n] en $v0, primero se suma el valor de 0 con la direccion de $t3 y luego se lleva a $v0 para ejecutar lectura y recuperacion (en algunos casos)
    jr $ra		  # Retornamos la direccion guardada en $ra
